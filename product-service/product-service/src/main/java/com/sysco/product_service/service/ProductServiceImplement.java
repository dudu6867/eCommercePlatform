...
public class ProductServiceImplement implements ProductService{
    ...
    public ProductDTO getProductByCategory(Long categoryId) {
        Product product = productRepository.findById(categoryId)
                .orElseThrow(() -> {
                    logger.info("Product not found for category id: {}", categoryId);
                    return new IdNotFoundException("Invalid Product Category ID");
                });

        logger.info("Query product by category id: {} successful", categoryId);
        return productMapper.toDto(product);
    }

    public ProductDTO getProductBySupplier(Long supplierId) {
        Product product = productRepository.findById(supplierId)
                .orElseThrow(() -> {
                    logger.info("Product not found for supplier id: {}", supplierId);
                    return new IdNotFoundException("Invalid Product Supplier ID");
                });

        logger.info("Query product by supplier id: {} successful", supplierId);
        return productMapper.toDto(product);
    }
